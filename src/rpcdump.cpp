...
Value importprivkey(const Array& params, bool fHelp) {
    ...
    if(fHelp || (params.size() < 1) || (params.size() > 3))
      throw(runtime_error(
        "importprivkey <swisscoin-classic_privkey> [label] [rescan]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <swisscoin-classic_address>\n"
            "Reveals the private key corresponding to <swisscoin-classic_address>.");
