...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    if(!pindexPoS) {
        ...
        //obj.push_back(Pair("posreward",     (float)(GetProofOfStakeReward((int)1, (int64)NULL, (int64) 1))/COIN));
    } else {
    	//CBlock block;
    	//ReadBlockFromDisk(block, pindexPoS);
    	//uint64 nCoinAge;
        //uint64 nCoinAgeFails;
       // block.GetCoinAge(&nCoinAge, &nCoinAgeFails);
        obj.push_back(Pair("posdifficulty", (double)GetDifficulty(pindexPoS)));
        //obj.push_back(Pair("posreward",     (float)(GetProofOfStakeReward(pindexPoS->nHeight, (int64)NULL),nCoinAge)/COIN));
    }
...
Value getwork(const Array& params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Swisscoin-Classic is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Swisscoin-Classic is downloading blocks...");
