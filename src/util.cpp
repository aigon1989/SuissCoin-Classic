...
#else
    const char* pszModule = "swisscoin-classic";
#endif
    if (pex)
        return strprintf(
...
#else
    /* Linux, Mac OS X, *BSD and so on: ~/.swisscoin-classic */
    char* pszHome = getenv("HOME");
    if(!pszHome || !strlen(pszHome))
      /* Must be root if no $HOME set */
#if (__APPLE__)
      path = fs::path("/private/var/root/.swisscoin-classic");
#else
      path = fs::path("/root/.swisscoin-classic");
#endif
    else
      path = fs::path(pszHome) / ".swisscoin-classic";
#endif
...
boost::filesystem::path GetConfigFile()
{
    ...
    else pathConfigFile = fs::path("swisscoin-classic.conf");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "swisscoin-classicd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Swisscoin-Classic will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("Swisscoin-Classic"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
